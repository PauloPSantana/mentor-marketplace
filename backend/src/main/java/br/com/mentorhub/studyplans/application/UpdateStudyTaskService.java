...
@Service
public class UpdateStudyTaskService {
    ...
    @Transactional
    public StudyPlanResponse execute(UUID actorUserId, UUID mentorshipId, UUID taskId, UpsertStudyTaskRequest request) {
        ...
        studyTaskRepository.save(task.update(
                request.title(),
                request.description(),
                request.taskType(),
                request.orderNumber() == null ? task.getOrderNumber() : request.orderNumber(),
                request.dueDate(),
